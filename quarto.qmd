...
Esto concluye a que tener acceso a internet está asociado a una mayor probabilidad de aprobar, la diferencia es de **7.8** puntos porcentuales.
...
Para las puntuaciones de failure = 2 o 3, la distribución es mucho más dispersa y se extiende hacia notas muy bajas, llegando incluso a acumularse en valores cercanos a 0, lo que muestra una alta probabilidadkmln de no aprobar.
...
El modelo elegido es Random Forest ya que nos parece un excelente modelo predictor.
...
test = testing(split)
modelo = rand_forest(trees= 1000) |>  set_engine("ranger", importance = "impurity") |> 
  set_mode("regression") |> fit(G3 ~ ., data = train)
modelo |> vip()
```
...
A continuacion haremos un modelo teniendo en cuenta `G1` y `G2`.
...
test = testing(split)
modelo = rand_forest(trees= 1000) |>  set_engine("ranger", importance = "impurity") |> 
  set_mode("classification") |> fit(Estado ~ ., data = train)
modelo |> vip()
```
...
Vemos que el modelo mejora notablemente de nuevo, pasa a una precision de 90% y estima aproximadamente 9 de cada 10 casos correctamente.

Hay algo que en todos los modelos coincide, que es que las variables más importantes para predecir la nota final, sin tener en cuenta `G1` y `G2`, son:

-   Failure: Número de cursos reprobados previamente (0, 1, 2, 3, 4 que serian 4 o +)

-   Absences - Número de inasistencias (de 0 a 93)

## Comentarios finales Principales conclusiones
...
Cambiar el enfoque a un modelo de clasificación (aprobado/no aprobado) permitió mejorar la precisión. Sin G1 y G2, la precisión alcanzó un 73%, mientras que al incluirlas se elevó a un 90%, lo que refuerza su relevancia.
